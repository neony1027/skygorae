...
- `src/*.css` — theme source, split by area. Index:
  ...
  - `colors.css` — overrides of Discord's `--*` design-system variables, mapped to skygorae palette. Most "wrong color" bugs are fixed here.
...
- `themes/skygorae.theme.css` — user-facing entry: variables + `@import` of the build.
...
- `build/skygorae.css` — generated from `src/*.css` by the build tooling and committed to git. **Don't hand-edit.** User `.theme.css` files `@import` the build copy of this.
...
- Edit `src/*.css`, never `build/skygorae.css`.
...
- User-facing CSS variables (colors, sizes, toggles) are defined in `themes/skygorae.theme.css`. The flavor files are standalone copies: keep their public variable interface in sync when adding or renaming variables, while preserving flavor-specific values. Structural rules live in `src/*.css`.
